...
I propose a solution: why don't we use the massive amount of user generated data of social networks to try and understand what are the reasons a user _feels_ a certain way towards a movie. Instead of using the small sample size of the users on movie review sites, let's include a larger, more diverse and a more representive sample of what the average paying customer that drives films success. In addition, we might be able to extract what elements of movies makes it more likely for a good review.
...
```{r, echo=FALSE}
# Sentiment per Tweet
aggtwitter <- aggregate(sent_combo$sentiment, by=list(sent_combo$element_id), FUN=mean)
...
Here we see the sentiment score ploted across all of our tweets. We observe a few crazy scores here and there but for the most part we see that most tweets are neutral.

```{r,echo=FALSE}
#histogram
ggplot(data=aggtwitter, aes(x))+geom_histogram(bins = 10, fill="#EA1313", color = "grey") + theme_dark() + labs(x="Sentiment Score",y="Count")


hchart(aggtwitter$x, color = "#B71C1C", name = "Price") %>% 
  hc_title(text = "You can zoom me")

h2 <- hist(aggtwitter$x, breaks = 20, plot = FALSE)
hchart(h2)
``` 
Feel free to zoom into the histogram.

This histogram comfirms our previous point.
...
ggplot(df,aes(sent_syuzhet))+geom_histogram(bins = 10)
...
#line graph
ggplot(data=aggtwitter2, aes(x=Group.1, y=x)) + geom_bar(stat="identity") +
ggtitle("Sentiment per Tweet") + labs(x="Tweet", y="Sentiment Score") + theme_solarized_2(light = FALSE)

#histogram
ggplot(data=aggtwitter2, aes(x))+geom_histogram(bins = 10) +theme_solarized_2(light = FALSE)
``` 

```{r, eval=FALSE}
sent_syuzhet2 <- get_sentiment(final2$text, method="syuzhet")     # method="bing", "afinn", "nrc", "stanford" 

df = data.frame(sent_syuzhet2)
ggplot(df,aes(sent_syuzhet2))+geom_histogram(bins = 10)
mean(sent_syuzhet2)     # average sentiment
```

```{r, eval=FALSE}
sent_nrc2 <- syuzhet::get_nrc_sentiment(final2$text)     # Get more detailed sentiment.

sentsum2 <- colSums(sent_nrc2)
df_nrc2 = data.frame(sentsum2)
df_nrc2$sentiments = rownames(df_nrc2)

ggplot(df_nrc2, aes(sentiments,sentsum2)) + geom_col()
ggplot(df_nrc2[9:10,], aes(sentiments,sentsum2)) + geom_col()
``` 
