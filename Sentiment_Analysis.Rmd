...
Finally, we find that average sentiment falls at 0.0562 which is slightly but not greatly positive. 
...
#histogram
ggplot(data=aggtwitter, aes(x))+geom_histogram(bins = 10) +theme_solarized_2(light = FALSE)
``` 
...
Now let's go a little further than just positive and negative.
```{r, eval=FALSE}
sent_nrc <- syuzhet::get_nrc_sentiment(final$text)     # Get more detailed sentiment.
head(sent_nrc)
...
ggplot(df_nrc, aes(sentiments,sentsum)) + geom_col()
ggplot(df_nrc[9:10,], aes(sentiments,sentsum)) + geom_col()
``` 
...
ggplot(df_nrc2, aes(sentiments,sentsum)) + geom_col()
ggplot(df_nrc2[9:10,], aes(sentiments,sentsum)) + geom_col()
``` 